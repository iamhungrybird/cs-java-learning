...
[沉默王二](https://tobebetterjavaer.com/about-the-author/)2022年3月27日Java核心JVMJava大约 28 分钟
...
### [#](https://tobebetterjavaer.com/jvm/how-jvm-run-zijiema-zhiling.html#一-运行时栈帧结构)一. 运行时栈帧结构
...
![img](http://cdn.tobebetterjavaer.com/tobebetterjavaer/images/jvm/how-jvm-run-zijiema-zhiling-a58ee82e-c0b0-4c06-9606-f7a0f0df0de9)

#### [#](https://tobebetterjavaer.com/jvm/how-jvm-run-zijiema-zhiling.html#局部变量表)局部变量表
...
![img](http://cdn.tobebetterjavaer.com/tobebetterjavaer/images/jvm/how-jvm-run-zijiema-zhiling-70ab6bf6-4fbb-4722-99b4-a93d5061630c.png)
...
![img](http://cdn.tobebetterjavaer.com/tobebetterjavaer/images/jvm/how-jvm-run-zijiema-zhiling-e5e6037c-9be1-472f-8ab3-2754466e7828.png)
...
![img](http://cdn.tobebetterjavaer.com/tobebetterjavaer/images/jvm/how-jvm-run-zijiema-zhiling-6734774b-376c-49bf-a915-508c7e829557.png)

为什么等于 4 呢？带上 this 也就 3 个呀？

![img](http://cdn.tobebetterjavaer.com/tobebetterjavaer/images/jvm/how-jvm-run-zijiema-zhiling-91ad04f8-1620-44c9-83d1-6fbd7860701a.png)

查看 LocalVaraiableTable 就明白了，变量 i 的下标为 3，也就意味着变量 d 占了两个槽。

![img](http://cdn.tobebetterjavaer.com/tobebetterjavaer/images/jvm/how-jvm-run-zijiema-zhiling-630b50e3-fc37-4748-8d20-852d5358f87a.png)

#### [#](https://tobebetterjavaer.com/jvm/how-jvm-run-zijiema-zhiling.html#操作数栈)操作数栈
...
![img](http://cdn.tobebetterjavaer.com/tobebetterjavaer/images/jvm/how-jvm-run-zijiema-zhiling-f790aa0f-d742-465b-91bf-5f143ee098c1.png)

这是因为调用成员方法的时候会将 this 和所有参数压入栈中，调用完毕后 this 和参数都会一一出栈。通过 「Bytecode」 面板可以查看到对应的字节码指令。

![img](http://cdn.tobebetterjavaer.com/tobebetterjavaer/images/jvm/how-jvm-run-zijiema-zhiling-c37add5c-a74b-4bd6-8c8e-9085d9e6d374.png)
...
![img](http://cdn.tobebetterjavaer.com/tobebetterjavaer/images/jvm/how-jvm-run-zijiema-zhiling-49e3f396-7ea8-49f5-81d4-093b9bdfa453.png)
...
![img](http://cdn.tobebetterjavaer.com/tobebetterjavaer/images/jvm/how-jvm-run-zijiema-zhiling-3ffdbe03-c0e4-49de-97a6-76666964a087.png)
...
#### [#](https://tobebetterjavaer.com/jvm/how-jvm-run-zijiema-zhiling.html#动态链接)动态链接
...
![img](http://cdn.tobebetterjavaer.com/tobebetterjavaer/images/jvm/how-jvm-run-zijiema-zhiling-93a21aaf-ff67-445d-8ddb-ac6f72fd9b25.png)
...
> ①、找到操作数栈顶的元素所指向的对象的实际类型，记作 C。 ②、如果在类型 C 中找到与常量池中的描述符匹配的方法，则进行访问权限校验，如果通过则返回这个方法的直接引用，查找结束；否则返回 `java.lang.IllegalAccessError` 异常。 ③、否则，按照继承关系从下往上一次对 C 的各个父类进行第二步的搜索和验证。 ④、如果始终没有找到合适的方法，则抛出 `java.lang.AbstractMethodError` 异常。
...
#### [#](https://tobebetterjavaer.com/jvm/how-jvm-run-zijiema-zhiling.html#方法返回地址)方法返回地址
...
#### [#](https://tobebetterjavaer.com/jvm/how-jvm-run-zijiema-zhiling.html#附加信息)附加信息

虚拟机规范允许具体的虚拟机实现增加一些规范里没有描述的信息到栈帧中，例如与调试相关的信息，这部分信息完全取决于具体的虚拟机实现。实际开发中，一般会把动态连接、方法返回地址与其他附加信息全部归为一类，成为栈帧信息。

### [#](https://tobebetterjavaer.com/jvm/how-jvm-run-zijiema-zhiling.html#二-方法调用)二. 方法调用
...
#### [#](https://tobebetterjavaer.com/jvm/how-jvm-run-zijiema-zhiling.html#解析)解析
...
#### [#](https://tobebetterjavaer.com/jvm/how-jvm-run-zijiema-zhiling.html#分派)分派
...
### [#](https://tobebetterjavaer.com/jvm/how-jvm-run-zijiema-zhiling.html#三-基于栈的字节码解释执行引擎)三. 基于栈的字节码解释执行引擎

虚拟机如何调用方法已经介绍完了，下面我们来看看虚拟机是如何执行方法中的字节码指令的。

#### [#](https://tobebetterjavaer.com/jvm/how-jvm-run-zijiema-zhiling.html#解释执行)解释执行
...
![img](http://cdn.tobebetterjavaer.com/tobebetterjavaer/images/jvm/how-jvm-run-zijiema-zhiling-3c8a0865-2a77-464e-8dd6-5616fd6a72d7.png)
...
#### [#](https://tobebetterjavaer.com/jvm/how-jvm-run-zijiema-zhiling.html#基于栈的指令集与基于寄存器的指令集)基于栈的指令集与基于寄存器的指令集
