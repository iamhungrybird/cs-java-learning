...
![两种并发模型的比较](images\两种并发模型的比较.png)
...
先谈一下运行时数据区，下面这张图相信大家一点都不陌生：![Java运行时数据区域](images\Java运行时数据区.png)
...
![正确同步图](images\正确同步.png)

操作的执行整体上有序，并且两个线程都只能看到这个执行顺序。

假设**没有使用同步**，那么在**顺序一致性模型**中的执行效果如下所示：

![没有正确同步图](images\没有正确同步.png)
...
![volatile内存示意图](images\volatile内存示意图.jpg)
...
![java对象头](images\watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2tleV94eWVz,size_16,color_FFFFFF,t_70)
...
![acquire流程](images\acquire流程.jpg)
